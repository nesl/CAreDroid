...
void dvmOptimizeClass(ClassObject* clazz, bool essentialOnly)
{
    int i;
    //salma
   // ALOGD("Salma: Optimize.cpp: dvmOptimizeClass");
    //endsalma
        ...
        if (!matched && !essentialOnly) {
            ...
            case OP_INVOKE_VIRTUAL:
                if (!rewriteExecuteInline(method, insns, METHOD_VIRTUAL)) {
                    ...
                    //salma
                	//__android_log_print(ANDROID_LOG_DEBUG, "DVM DEBUG", "Optimize.cpp: (!matched && !essentialOnly)OP_INVOKE_VIRTUALmethod name = %s",method->name);
                	//salma
                }
                break;
...
//    //salma
//    //methodIdx is the method number in the method constant pool the number BBBB
//  //  Method* baseMethodZero;
//    std::string methodname = method->name;
//   // u2 methodIdxZero = insns[2]; // first one
// //   baseMethodZero = dvmOptResolveMethod(clazz, methodIdxZero, METHOD_VIRTUAL, NULL);
// //   if(methodname.find("myadd") != std::string::npos){
//  //  	  __android_log_print(ANDROID_LOG_DEBUG, "DVM DEBUG", "baseMethodZero = %s, clazz = %s, method idzero = %d",baseMethodZero->name,method->clazz->descriptor,methodIdxZero);
//
// //      }
//    //endsalma
   ...
   //method id --> is the id of the next method executed in myadd
//   if(methodname.find("myadd") != std::string::npos || methodname.find("onCreate") != std::string::npos){
//	  __android_log_print(ANDROID_LOG_DEBUG, "DVM DEBUG", "Optimize.cpp: rewriteVirtualInvoke method name = %s, baseMethod = %s, baseMethodIndex = %d, clazz = %s, method idx = %d, method index = %d, virtualMethodCount = %d, vtableCount=%d ",method->name,baseMethod->name,baseMethod->methodIndex,method->clazz->descriptor,methodIdx,method->methodIndex,method->clazz->virtualMethodCount,method->clazz->vtableCount);
//
//   }
    //endsalma
    if (baseMethod == NULL) {
    ...
    MethodType methodType)
{
    ...
    //salma
   //  __android_log_print(ANDROID_LOG_DEBUG, "DVM DEBUG", "Optimize.cpp: rewriteExecuteInline method name = %s, baseMethod = %s",method->name, baseMethod->name);
    //salma
    if (calledMethod == NULL) {
