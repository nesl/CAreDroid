...
{
	//salma
	//ALOGD("Entry to portable interpreter");
	//endsalma
#if defined(EASY_GDB)
    ...
    //salma
    //this print is captured
     //   __android_log_print(ANDROID_LOG_INFO, "DVM DEBUG", "threadid=%d: %s.%s pc=%#x fp=%p",
     //           self->threadId, curMethod->clazz->descriptor, curMethod->name,
     //           pc - curMethod->insns, fp);
    //endsalma
    /*
...
//salma
  //  ALOGD("|-- Now interpreting %s.%s", curMethod->clazz->descriptor,
  //          curMethod->name);
 //endsalma
#if 0
...
GOTO_TARGET(invokeVirtual, bool methodCallRange, bool)
    {
        ...
        //salma
        //ALOGD("invoke virtual: method to call %s",methodToCall->name );
        //endsalma
...
//salma
        ...
        /*Use of replaceable table with switch flag got from policy*/
//        if(dvmSystemGetDefaultSwitchPolicy()){ // Return the current switchflag
//        	ALOGD("Switch is enabled");
//        	if(methodToCall->isReblaceable){
//        		if(methodToCall->priority < thisPtr->clazz->repMethodCount){ //meaning there still less priority method
//        			methodToCall =  thisPtr->clazz->reptable[methodToCall->priority + 1 ];
//        		}
//        	}
//        }
...
//         }
        ...
        /*** Hack the OP code to change the method called ***/
        // Salma: TODO Comment for now just for testing the power of context
        if(methodToCall->isReblaceable){
        		//clock_t begin = clock();
				...
				//ALOGD("(GOTO_TARGET)Elapsed time for decision in sec: %f\n",elapsed_secs);
				//ALOGD("Method is replaced by:  %s.%s",thisPtr->clazz->descriptor, methodToCall->name);
        }



        //salma //direct change of the method
//if(SWITCH_ENABLED){
//        std::string methodname=methodToCall->name;
//
//        if(methodname.find("solve_ChDec") != std::string::npos )
//        {
//        	// __android_log_print(ANDROID_LOG_DEBUG, "DVM DEBUG","solve_ChDec, ref= %d",ref);
//           if (ref == 13)
//           {
//
//       // 	switchloop++;
//        	//   __android_log_print(ANDROID_LOG_DEBUG, "DVM DEBUG", "loop=%d",switchloop);
//       // 	   if(switchloop>4)
//       // 		   {
//        		   // Do the switch to solve_CG
//        		   	   ref = 12;
//        		   	   methodToCall = thisPtr->clazz->vtable[ref];
//        //		   }
//
//         	 }
//          // __android_log_print(ANDROID_LOG_DEBUG, "DVM DEBUG","+++ virtual[%d]=%s.%s",
