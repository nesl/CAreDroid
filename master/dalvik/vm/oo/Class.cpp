...
//salma
//static bool createReptable(ClassObject* clazz); //not used anymore
//endsalma
...
static bool prepareCpe(ClassPathEntry* cpe, bool isBootstrap)
{
    ...
    if ((strcmp(suffix, "jar") == 0) || (strcmp(suffix, "zip") == 0) ||
        ...
        if (dvmJarFileOpen(cpe->fileName, NULL, &pJarFile, isBootstrap) == 0) {
            ...
            cpe->ptr = pJarFile;
            //salma
           // ALOGD("vm/oo/Class.cpp: open the dvmJarFileOpen cpe->filename: %s", cpe->fileName);
            //endsalma
            return true;
        }
    ...
    const u1* pEncodedData, Object* classLoader)
{
    ...
    if (pHeader->virtualMethodsSize != 0) {
        ...
        for (i = 0; i < count; i++) {
            dexReadClassDataMethod(&pEncodedData, &method, &lastIndex);
            //salma
          //  ALOGD("Class.cpp: newClass name= %s, classIndex=%d, lastIndex(methodFlatID) = %d, virtualmethodindex=%d",newClass->descriptor, classDefIdx,lastIndex, i);
            //here I want to map flat methodIdx(last index) with the virtual count
            bool isMapMethodIDUpdate = false;
            //u4 methPriority = 0;

            //cleaned out the priority map
//            if(pDvmDex->pconfigMap != NULL){
//             //just check
//            	//ALOGD("Before Replacing");
//            	//For Debug --> Remove later
//            //	dvmDexConfigMapDebug(pDvmDex->pconfigMap);
//            	//ALOGD("Replacing the flat method ID into its number in the virtual methods count");
//            	isMapMethodIDUpdate=dvmUpdateConfigMapID(pDvmDex->pconfigMap, classDefIdx, lastIndex, (u4)i);
//            	//For Debug --> Remove later
//            	//dvmDexConfigMapDebug(pDvmDex->pconfigMap);
//            	//ALOGD("After Replacing");
//            }


         //   if(pDvmDex->pconfigMethodMap != NULL){
          //  	ALOGD("Before Replacing ");
          //  	dvmConfigMethodMapDebug(pDvmDex->pconfigMethodMap);
         //   }
            if(pDvmDex->pconfigMethodMap != NULL){
                isMapMethodIDUpdate=dvmUpdateConfigMapMethodID(pDvmDex->pconfigMethodMap, classDefIdx, lastIndex, (u4)i);
              //  ALOGD("After Replacing");
              //  dvmConfigMethodMapDebug(pDvmDex->pconfigMethodMap);
            }
            ...
            //salma
            if(isMapMethodIDUpdate){
            	newClass->virtualMethods[i].isReblaceable = true;
            	//clean up priority only
            	//methPriority = dvmGetMethodPriority(pDvmDex->pconfigMap,classDefIdx, (u4)i);
            	//newClass->virtualMethods[i].priority = methPriority;
            	//ALOGD("Priority:%d  for method:%d ",newClass->virtualMethods[i].priority, i);

            	dvmGetMethodParam(pDvmDex->pconfigMethodMap,classDefIdx, (u4)i, newClass->virtualMethods[i].methodparam );
            	if(newClass->virtualMethods[i].methodparam != NULL)
            		newClass->virtualMethods[i].priority = newClass->virtualMethods[i].methodparam->priority;
            }
            else{
            	newClass->virtualMethods[i].isReblaceable = false;
            	newClass->virtualMethods[i].priority = 0;//default one
    ...
    Method* meth)
{
    ...
    //salma
   // ALOGD("Class.cpp.loadMethodFromDex:clazz=%s, methodId = %u, methodname=%s",clazz->descriptor,pDexMethod->methodIdx,meth->name );
    //endsalma
...
//xxx is this the right exception?  better than an assertion.
    ...
    if (dvmIsInterfaceClass(clazz)) {
        ...
        //salma
        //not used anymore
//        if(!createReptable(clazz)) {
//        	ALOGE("failed creating reptable");
//        }
        //endsalma
    }
        ...
        for (i = 0; i < clazz->virtualMethodCount; i++) {
            ...
            for (si = 0; si < clazz->super->vtableCount; si++) {
                ...
                if (dvmCompareMethodNamesAndProtos(localMeth, superMeth) == 0) {
                    ...
                    //salma
                 clazz->isSensitiveClass = false; //default

                 if(clazz->pDvmDex->pconfigMethodMap !=NULL){
                	 //Compare current class with the list of sensitive classes in configuration file
                	 if(dvmIsClassSensitive(clazz)){
                		 clazz->isSensitiveClass = true;
                		 //Mark the thread ID
                         u4 curThreadId = clazz->initThreadId;
                       //  ALOGD("Class.cpp: Check the current thread for sensitive class = %d", curThreadId);
                         Thread* curThread = dvmGetThreadByThreadId(curThreadId);
                		 curThread->isSensitive = true;
                		 // pDexClassDef = dexFindClass(clazz->pDvmDex->pDexFile,clazz->pDvmDex->pConfigFile->sensitiveClasses.at(i/*this is the problem*/).className.c_str() );
                		 pDexClassDef = dexFindClass(clazz->pDvmDex->pDexFile,clazz->descriptor);
						 if(pDexClassDef==NULL){
							// ALOGD("vtable create override methods: pDexClassDef == NULL");
						 }
						 else{
							u4 classIdx = dexGetIndexForClassDef(clazz->pDvmDex->pDexFile,pDexClassDef);
							//ALOGD("vtable Override: current method id = %d, methodname = %s, vref = %d",i,localMeth->name,si);
							//dvmUpdateConfigMapID(clazz->pDvmDex->pconfigMap, classIdx, (u4)i, (u4)si); //cleaned out
							dvmUpdateConfigMapMethodID(clazz->pDvmDex->pconfigMethodMap, classIdx, (u4)i, (u4)si);
							clazz->classID = classIdx;

							//clazz->repMethodCount = dvmConfigMapNumOfSensitiveMethodsInClass(clazz->pDvmDex->pconfigMap, classIdx); //cleaned out. not used anymore
							//For Debug --> remove later
							//ALOGD("After replacing with vref");
							//dvmConfigMethodMapDebug(clazz->pDvmDex->pconfigMethodMap);
						 }
                	 }
                 }

                // ALOGD("override %s.%s (slot %d)",
                ...
                if(clazz->pDvmDex->pconfigMethodMap !=NULL){
            	 ...
            	 if(dvmIsClassSensitive(clazz)){
            		...
            		// ALOGD("Class.cpp: Check the current thread for sensitive class = %d", curThreadId);
            		 ...
            		 pDexClassDef = dexFindClass(clazz->pDvmDex->pDexFile,clazz->descriptor);
					 if(pDexClassDef==NULL){
						 ALOGE("vtable add to end: pDexClassDef == NULL");
					 }
					 else{
						u4 classIdx = dexGetIndexForClassDef(clazz->pDvmDex->pDexFile,pDexClassDef);
						//dvmUpdateConfigMapID(clazz->pDvmDex->pconfigMap, classIdx, (u4)i, (u4)localMeth->methodIndex); //cleaned out
						dvmUpdateConfigMapMethodID(clazz->pDvmDex->pconfigMethodMap, classIdx, (u4)i, (u4)localMeth->methodIndex);
						clazz->classID = classIdx;

						//clazz->repMethodCount = dvmConfigMapNumOfSensitiveMethodsInClass(clazz->pDvmDex->pconfigMap, classIdx); //not used anymore //cleanedout

						//For Debug --> remove later
						//ALOGD("vtable add to end: class: %d current method id = %d, methodname = %s, vref = %d",classIdx,i,localMeth->name,actualCount);
						//ALOGD("After replacing with vref");
						//dvmConfigMethodMapDebug(clazz->pDvmDex->pconfigMethodMap);
					 }
            	 }
            	// else{
            		// ALOGD("clazz %s not sensitive  add method %s,  %d, previousID = %d", clazz->descriptor, localMeth->name, localMeth->methodIndex, i);
            	 //}

             }
           //  else{
            //	 ALOGD("clazz->pDvmDex->pconfigMap ==NULL , add method %s.%s,  %d, previousID = %d",  clazz->descriptor, localMeth->name, localMeth->methodIndex, i );
            // } //else(clazz->pDvmDex->pconfigMap ==NULL )


            // ALOGD("add method %s.%s,  %d",clazz->descriptor, localMeth->name, localMeth->methodIndex);
             //endsalma
                actualCount++;
...
/*
 * Create the replaceable method table. (not used anymore cleaned out)
 *
 * The top part of the table is a copy of the table from our superclass,
 * with our local methods overriding theirs.  The bottom part of the table
 * has any new methods we defined.
// */
//static bool createReptable(ClassObject* clazz)
//{
//    bool result = false;
//    int maxCount;
//    //int i;
...
//    u4 classIdx = 0;
//    int vtableref;
//    //int count = 1; //index in replaceable table starts with 1 because it will corresponds to priority
//    maxCount = clazz->repMethodCount;
//
//    if(!clazz->isSensitiveClass) return true;
//
//
//    DvmConfigMap* pDvmConfigMap = clazz->pDvmDex->pconfigMap;
//    if(pDvmConfigMap == NULL){
//    	ALOGD("createReptable pDvmConfigMap = null");
//    	return false;
//    }
//    /*
//     * Over-allocate the table, then realloc it down if necessary.  So
...
//     */
//    dvmLinearReadWrite(clazz->classLoader, clazz->virtualMethods);
//    clazz->reptable = (Method**) dvmLinearAlloc(clazz->classLoader,
//                        sizeof(Method*) * maxCount);
//    if (clazz->reptable == NULL)
//        goto bail;
//
//
//    pDexClassDef = dexFindClass(clazz->pDvmDex->pDexFile,clazz->descriptor);
//    if(pDexClassDef==NULL){
//    	 ALOGD("reptable pDexClassDef == NULL");
//    }
//    else{
//    classIdx = dexGetIndexForClassDef(clazz->pDvmDex->pDexFile,pDexClassDef);
//    }
//
//
//    for(DvmConfigMapIter it=pDvmConfigMap->begin(); it!=pDvmConfigMap->end(); ++it){
//    	if(it->first.first == classIdx){
//    	//	ALOGD("it->first.first == classIdx %d", classIdx );
//    		vtableref = it->first.second; //methodref == vtableref
//    		Method* localMeth = clazz->vtable[vtableref];
//    		// Methods are arranged in replaceable table based on priority
//    	//	ALOGD("method name: %s method priority = %d method id= %d", localMeth->name, localMeth->priority, localMeth->methodIndex);
//    		clazz->reptable[localMeth->priority] = localMeth;
//    		//count++;
//    	}
//    }
//    return true;
...
//
//bail:
//    dvmLinearReadOnly(clazz->classLoader, clazz->reptable);
//    return result;
//}
