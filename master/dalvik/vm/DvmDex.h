...
struct DvmDex {
    ...
    //salma
    /* pointer to the ConfigFile we're associated with if there is any */
    bool				isConfigFile;
    DvmConfigFile*		pConfigFile;

    // Map of method parameters
    //DvmConfigMap* 		 	pconfigMap; // Clean up .. this is for priority only
    DvmConfigMethodMap*	    pconfigMethodMap;
...
//salma
/*
 * Map the structures in the VM structures with the data(priority only) in ConfigFile
 * */
void dvmDexConfigMap(DvmDex* pDvmDex,  DvmConfigMap* pDvmConfigMap);
/*
 * Map the structures in the VM structures with the data(Method Parameters) in ConfigFile
 * */
...
//bool dvmDexUpdateConfigMapMethodID(DvmConfigMap* pDvmConfigMap,u4 classId, u4 previousMethodId, u4 updatedMethodId);
