...
//global variable
//u2 gWasWith_JIT;
//ExecutionMode gOriginal_Execution_Mode;
vector<u4> gSensitiveThreads;
//endsalma
    ...
    const Method* method, DvmDex* methodClassDex)
{
    ...
    //salma
    //__android_log_print(ANDROID_LOG_DEBUG, "DVM DEBUG", "method idx = %d\n", methodIdx);
 //   std::string methodname = methodToCall->name;
  //  if(methodname.find("myadd") != std::string::npos)
 //   {
   // 	__android_log_print(ANDROID_LOG_DEBUG, "DVM DEBUG", "Interp.cpp: method name = %s",methodToCall->name);
    //	__android_log_print(ANDROID_LOG_DEBUG, "DVM DEBUG", "Interp.cpp: clazz name = %s",methodToCall->clazz->descriptor);
    //	__android_log_print(ANDROID_LOG_DEBUG, "DVM DEBUG", "Interp.cpp: super name = %s",methodToCall->clazz->super->clazz->descriptor);
   // }
	...
	//ALOGD("Inside dvmInterpret");
	 if(self->isSensitive){
		// ALOGD("In Interpreter thread is sensitive");
		 ...
		 // The location flag throws exception --> comment for now
//		 if (dvmSystemLocationGetLocationThreadFlag() == false){
//			 ALOGD("dvmSystemLocationGetLocationThreadFlag() == false"); //should appear only once in the log
//			 if(dvmSystemLocationDetectionStartup() == false)
//				 ALOGD("Location detection thread didn't start!!! ");
//		 }
...
//    	dvmSystemCoreValuesSetJniEnv(self->jniEnv);

	// 	 ALOGD("dvmInterpret: methodclazz=%s, methodname=%s, threadId=%d",method->clazz->descriptor, method->name, self->threadId);
	//ALOGD("dvmInterupt: %d", self->threadId);
//
//
//
//	if((gWasWith_JIT == 1) && (!dvmIsClassSensitive(method->clazz))){
//		ALOGD("Correction of interpreter for regular classes");
//		gWasWith_JIT = 0;
//		#ifndef WITH_JIT
//			ALOGD("JIT is not defined now"); // not captured
//			#define WITH_JIT
//			//debug //this is not captured
//				#if defined(WITH_JIT)
//							ALOGD("WITH_JIT is defined now");
//				#endif
//		#endif
//		#if defined(WITH_JIT)
//				ALOGD("JIT is already defined");
//		#endif
//		//enddebug
//		 gDvm.executionMode = kExecutionModeJit;
//		 ALOGD("Switch to JIT execution mode");
...
////salma
//    /*
//     * Everytime the interpreter is called on a new thread the system core states are checked to set the global switch flag
//     * Only if the class in sensitive
//     * */
//
//   if(dvmIsClassSensitive(method->clazz)){
//    	gDvm.executionMode = kExecutionModeInterpPortable;
//    }
//   else if (self->isSensitive && !dvmIsClassSensitive(method->clazz)) {
//    	//ALOGD("Class is not sensitive but thread is sensitive");
//   	gDvm.executionMode = kExecutionModeInterpPortable;
//    }
//    else if(!self->isSensitive && dvmIsClassSensitive(method->clazz)){
//    	ALOGD("BUG: This shouldn't happen");
//    }
//    else if (!self->isSensitive){
//    //	ALOGD("Not a sensitive thread switch to JIT");
//    	gDvm.executionMode = kExecutionModeJit;
//    }
////endsalma
...
//    { //salma
//    //	__android_log_print(ANDROID_LOG_DEBUG, "DVM DEBUG","interFast: dvmMterpStd");
//    	//endsalma
//    	stdInterp = dvmMterpStd;
//   }
//
...
//    {
//    	//salma
//    	//	__android_log_print(ANDROID_LOG_DEBUG, "DVM DEBUG","WITH_JIT %d: dvmMterpStd ", gDvm.executionMode);
//    	//endsalma
//        stdInterp = dvmMterpStd;
//    }
//#endif
...
//    {
//    	//salma
//    	// 	__android_log_print(ANDROID_LOG_DEBUG, "DVM DEBUG","dvmInterpretPortable");
//    	//endsalma
//    	stdInterp = dvmInterpretPortable;
//    }

    //make everything portable for now for debugging
    gDvm.executionMode = kExecutionModeInterpPortable;
