...
static int parse_location(){
	...
	std::string line;
	// this file is just for simulation ... it was saved from a run so that I don't have to wait
	// for the gps to power on and get its data everytime I boot the device
	// I saved this file under the main folder of CAreDroid
	// The original one is at /sdcard/location.txt --> This is the one that is written on the run
	// so comment this line in the final release
...
int reportLocation()
{
	ALOGD("Report location thread function");
    sp<IServiceManager> sm = defaultServiceManager();

    if(checkCallingPermission(String16("android.permission.DUMP")) == true)
