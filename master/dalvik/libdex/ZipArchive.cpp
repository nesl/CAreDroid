...
//salma
    //debug
   // __android_log_print(ANDROID_LOG_DEBUG, "DVM DEBUG", "ZipArchive: filename = %s , %p",fileName, pArchive);
//endsalma
...
ZipEntry dexZipFindEntry(const ZipArchive* pArchive, const char* entryName)
{
    ...
    while (pArchive->mHashTable[ent].name != NULL) {
    	//salma
    	//print all the the entries in the hash table
    	//ALOGD("ZipArchive.cpp: dexZipFindEntry: pArchive->mHashTable[%d].name=%s",ent, pArchive->mHashTable[ent].name);
//    	if(strstr(pArchive->mHashTable[ent].name,"config.xml") != NULL){
//    		ALOGD("Check values concerning with config.xml");
//    		ALOGD("pArchive->mHashTable[ent].nameLen == nameLen ==> %d= %d", pArchive->mHashTable[ent].nameLen, nameLen);
//    		ALOGD("size of pArchive->mHashTable[ent].name = %d ", sizeof(pArchive->mHashTable[ent].name));
//    		ALOGD("size of entryName %d", sizeof(entryName));
//    		int n =memcmp(pArchive->mHashTable[ent].name, entryName, nameLen);
//    		ALOGD("Result of memcmp: %d",n);
//    	}
